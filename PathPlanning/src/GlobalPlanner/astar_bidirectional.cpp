...
private:
    ...
    vector<vector<double>> calc_final_bidirectional_path(
        shared_ptr<Node> n1, shared_ptr<Node> n2, unordered_map<double, shared_ptr<Node>>& set1,
        unordered_map<double, shared_ptr<Node>>& set2);
...
vector<vector<double>> BidirectionalAStarPlanner::planning(double sx, double sy, double gx,
                                                           double gy) {
    ...
    while (true) {
        ...
        for (const vector<double>& m : get_motion()) {
            vector<shared_ptr<Node>> nodes = {
                std::make_shared<Node>(current_A->x + m[0], current_A->y + m[1],
            ...
            bool continue_A =
                (!verify_node(nodes[0]) || closed_set_A.find(n_ids[0]) != closed_set_A.end());
            bool continue_B =
                (!verify_node(nodes[1]) || closed_set_B.find(n_ids[1]) != closed_set_B.end());

            if (!continue_A && (open_set_A.find(n_ids[0]) == open_set_A.end() ||
                                open_set_A[n_ids[0]]->cost >= nodes[0]->cost)) {
                open_set_A[n_ids[0]] = nodes[0];
            }
            if (!continue_B && (open_set_B.find(n_ids[1]) == open_set_B.end() ||
                                open_set_B[n_ids[1]]->cost >= nodes[1]->cost)) {
                open_set_B[n_ids[1]] = nodes[1];
            }
        }
    }
    vector<vector<double>> path =
        calc_final_bidirectional_path(meet_point_A, meet_point_B, closed_set_A, closed_set_B);
